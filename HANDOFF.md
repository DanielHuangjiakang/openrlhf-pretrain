...
你不需要理解实验内容 —— 按顺序跑几条命令，把指定的东西发回来就行。
...
| | |
|---|---|
| GPU | 8 张（H800/A100/H100 均可），**实际占用约 10–15 小时** |
| 磁盘 | **约 250 GB**，在 `workspace/` 下 |
| 时间 | 总跨度约 12–18 小时，其中数据准备约 1 小时是**纯 CPU**，那段时间 GPU 空着 |
| 外网 | 需要（下载数据集和依赖） |
...
| 6 | `bash run.sh grpo` | 4–6 时 | **是** | `workspace/status.txt` |

**第 2 步过了之后，3–6 可以一条命令跑完，中间不需要任何操作：**
